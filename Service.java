...
public class Service {
    ...
    public Task doIteration() {
        ...
        if (taskTime < minDeviceEndTime) {
            ...
            Task resolvedTask = processDeviceResolveTaskEvent(device);
            return resolvedTask;
        }
        return null;
    }
...
//        }
    ...
    private Task processDeviceResolveTaskEvent(Device device) {
        Task task = serviceQueue.peek();

        Task complitedTask = completeOldTask(device);
        executeNewTask(device, task);
        return complitedTask;
    }

    private Task completeOldTask(Device device) {
        ...
        if (resolveTask != null) {
            if (nextService != null) {
                ...
                if(!resultQueue.contains(resolveTask))

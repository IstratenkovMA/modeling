...
public class TaskManager {
    ...
    public void run() {
        while (isContinueWorking()) {
            ...
            if (arriveTaskTime < serviceEvent) {
                ...
                Task possibleResolvedTask = service.doIteration();

                resovleSecondServiceProcess(service, possibleResolvedTask);
            } else {
        ...
        for (Task task : allResults) {
            if (task.isReturned()) {
                countReturned++;
                timeWorking += task.getSummaryTime();
                queueWaiting += task.getSummaryWaiting();
            } else {
                timeWorking += task.getFinishResolveTime() - task.getArriveTime();
                queueWaiting += task.getStartResolveTime() - task.getArriveTime();
            }
        }
        int countResolved = allResults.size() - countReturned;

        System.out.println("Полное время работы: " + currentTime);
        System.out.println("Количество returned: " + countReturned);
        System.out.println("Количество: " + allResults.size());
        System.out.printf("Процент возвращенных: %.2f%%\n", ((double) countReturned / allResults.size()) * 100);
    ...
    private void resovleSecondServiceProcess(Service service, Task possibleResolvedTask) {
        if(possibleResolvedTask != null && services.indexOf(service) == 1) {
            int p = Math.abs(random.nextInt() % 101);
            if(p <= K) {
                possibleResolvedTask.markReturned();
                services.get(0).addTask(possibleResolvedTask);
            }
        }
    }
